...
class SawyerDoorHookEnv(
    ...
    def reset_model(self,bias_flag = False):
        ...
        #print(goal)
        if bias_flag:
            goal = self.sample_bias_goal()
        self.set_goal(goal)
        self.reset_mocap_welds()
        return self._get_obs()

    def reset(self,bias_flag = False):
        # super.reset() does not account for reset-free logic.
        ob = self.reset_model(bias_flag = bias_flag)
    ...
    def sample_bias_goal(self):
        goal = np.random.uniform(self.goal_space.low,self.goal_space.high/2)
        return {'desired_goal':goal , 'state_desired_goal':goal}
