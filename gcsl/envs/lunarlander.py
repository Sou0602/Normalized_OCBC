...
class LunarEnv(GoalEnv, Serializable):
    ...
    def sample_goal(self,bias_flag=False):
        if bias_flag == False:
            ...
            g0_low = self.goal_space.low.flatten()
            g0_high = self.goal_space.high.flatten()
            g0_avg = (g0_low + g0_high)/2
            ...
            goal_state = np.array([0.0, 0, 0, 0, 0, 0, 1, 1,])

            obs_high = self.observation_space.low.flatten()
            obs_low = self.observation_space.high.flatten()
