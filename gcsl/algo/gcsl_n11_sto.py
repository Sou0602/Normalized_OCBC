...
class GCSL:
    ...
    def loss_fn(self, observations, goals, actions,imp_weights, horizons, weights):
        ...
        l = nll * weights_torch
        c_l = c_nll * weights_torch
        m_l = m_nll * weights_torch
        imp_weights = torch.tensor(imp_weights,dtype = torch.float32)

        # l = (1 - eps <= imp_weights) * (imp_weights <= 1 + eps) * l
        # c_l = (1 - eps <= imp_weights) * (imp_weights <= 1 + eps) * c_l
        # m_l = (1 - eps <= imp_weights) * (imp_weights <= 1 + eps) * m_l

        return torch.mean(l) , torch.mean(c_l) , torch.mean(m_l)

    def sample_trajectory_buffer(self, greedy=False, noise=0, render=False):
        if self.imbalanced_goals:
            goal_state = self.env.sample_goal(bias_flag = True)
        else:
            goal_state = self.env.sample_goal(bias_flag = False)
        ...
        relabel_state = states[-1]
        r_goal = self.env.extract_goal(relabel_state)
        w = []
        for t in range(len(actions)):
            ...
            horizon = np.arange(self.max_path_length) >= (
                        self.max_path_length - 1 - t)  # Temperature encoding of horizon
            probs_n = self.policy.probabilities(torch.tensor(observation[None],dtype=torch.float32),
                                                torch.tensor(goal[None],dtype=torch.float32),torch.tensor(horizon[None],dtype=torch.float32))
            probs_d = self.policy.probabilities(torch.tensor(observation[None],dtype=torch.float32),
                                                torch.tensor(r_goal[None],dtype=torch.float32),torch.tensor(horizon[None],dtype=torch.float32))
            weight = probs_n[0][action]/probs_d[0][action]

            w.append(weight.cpu().detach().numpy())

        w = np.array(w)
        w_prod = np.cumprod(w)
        self.imp_weights.append(w_prod[-1])
        #pdb.set_trace()
        return np.stack(states), np.array(actions), goal_state , w_prod[-1]
    ...
    def take_policy_step(self, buffer=None):
        ...
        for _ in range(self.n_accumulations):
            observations, actions, goals,com_goals,imp_weights, _, horizons, weights = buffer.sample_batch(self.batch_size)
            loss , c_loss , m_loss = self.loss_fn(observations, goals, actions,imp_weights, horizons, weights)
    ...
    def validation_loss(self, buffer=None):
        ...
        avg_m_loss = 0
        for _ in range(self.n_accumulations):
            observations, actions, goals,com_goals,imp_weights, lengths, horizons, weights = buffer.sample_batch(self.batch_size)
            loss, c_loss , m_loss = self.loss_fn(observations, goals, actions,imp_weights, horizons, weights)
            avg_loss += ptu.to_numpy(loss)
            avg_c_loss += ptu.to_numpy(c_loss)
            avg_m_loss += ptu.to_numpy(m_loss)
        ...
        with tqdm.tqdm(total=self.eval_freq, smoothing=0) as ranger:
            while total_timesteps < self.max_timesteps:
                ...
                if self.is_offline:
                    if self.replay_buffer.current_buffer_size < self.replay_buffer.max_buffer_size:
                        if total_timesteps < self.explore_timesteps:
                            states, actions, goal_state,w = self.sample_trajectory_buffer(noise=1)
                        else:
                            states, actions, goal_state,w = self.sample_trajectory_buffer(greedy=False, noise=self.expl_noise)
                        ...
                        if self.validation_buffer is not None and np.random.rand() < 0.2:
                            self.validation_buffer.add_trajectory(states, actions, goal_state,w)
                        else:
                            self.replay_buffer.add_trajectory(states, actions, goal_state,w)
                else:
                    #print('Not online')
                    # Interact in environmenta according to exploration strategy.
                    if total_timesteps < self.explore_timesteps:
                ...
                if timesteps_since_eval >= self.eval_freq:
                    ...
                    last_time = time.time()
                    logger.dump_tabular()
                    np.savetxt(logger.get_snapshot_dir() + 'imp_w_biased.txt',
                               np.array(self.imp_weights),delimiter=',')
                    # Logging Code
